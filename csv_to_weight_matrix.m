function [ A, B ] = csv_to_weight_matrix(data,index)

% convert UN Comtrade commodity data (.csv format) to matrix

% INPUT: 
% data - UN comtrade data: import .csv as a table <-- important!
% index - list of UN country codes selected (i.e., 156 = China...)

% OUTPUT:
% A - weighted adjacency matrix of trade (measured in kg)
...
for i = 1:numel(unique(data.Year))
    ...
    data.Netweightkg(isnan(data.Netweightkg)) = 0 ; % clear NaNs
    Adj(linear_ind) = data.Netweightkg(y);                        % add value of trade
    A(:,:,i) = Adj;
    B(:,:,i) = Adj > 0;
end
